...
async function fetchHdobFiles(stormDir) {
    ...
    if (response.ok) {
      ...
      links.each((i, link) => {
        ...
        if (href && href.endsWith('.txt')) {
          ...
          const timeDiff = (now - fileDate) / (1000 * 60 * 60); // Difference in hours
...
function extractHdobEntries(rawData) {
  const hdobEntries = [];
...
function convertUnits(value, type, unit) {
  if (unit === 'imperial') {
    if (type === 'temperature') {
...
function convertToPlacefile(hdobEntries) {
...
function savePlacefile(placefileData) {
